...
选择平台（TikTok / Kwai）
    ...
    ├─ 有已授权账号 ──▶ 广告主账号列表
    │                       │
    └─ 无账号 ──▶ 右上角「授权」──▶ OAuth 授权页（WebView）
                                        │
                                        ▼
                                    授权成功
                                        │
                                        ▼
                            后台自动同步：广告主/推广系列/广告组/广告
                                        │
                                        ▼
                                  广告主账号列表
                                        │
...
App
├── LoginView                      # 邮箱 + 密码登录
├── PlatformSelectView             # 选择平台（TikTok / Kwai）
├── AccountListView                # 广告主账号列表
│   ├── 搜索栏
│   ├── 下拉加载更多
│   ├── 余额查看 Sheet
│   └── 右上角「去授权」按钮
├── OAuthWebView                   # ASWebAuthenticationSession 授权
├── SyncProgressView               # 授权后自动同步进度提示
└── AccountDetailView              # 账号详情（3 Tab）
    ├── CampaignListView           # 推广系列列表
    │   ├── BudgetEditSheet        # 修改预算弹窗
    │   ├── StatusToggleAlert      # 开启/暂停确认
    │   └── AdGroupListView        # 点击进入（筛选当前系列）
    ├── AdGroupListView            # 广告组列表
    │   ├── BudgetEditSheet
    │   ├── StatusToggleAlert
    │   └── AdListView             # 点击进入（筛选当前广告组）
    └── AdListView                 # 广告列表（含搜索）
```
